...
o T.MONTH_ID = '$v_month_id' 
   and t.day_id<='$v_day_id'-----------------
   and t.VISIT_AREA_CODE=t1.area_code         
 GROUP BY t.USER_ID,t.device_number,t1.PROVINCE_NAME--,a.IMEI
;
...
count(distinct t.day_id) stay_days 
  ...
  inner join ( SELECT *
   FROM DIM.DIM_PUB_CODE_AREA_CODE T ) t1
onT.MONTH_ID = '$v_month_id'
and t.day_id<='$v_day_id' --------------------
and t.CALL_AREA_ID=t1.area_code      
 GROUP BY t.USER_ID,t.device_number,t1.PROVINCE_NAME--,t.IMEI
) a left join (
--ocs 
...
inner join ( SELECT *
   FROM DIM.DIM_PUB_CODE_AREA_CODE T) t1 
on .visit_area_code=t1.area_code  
...
insert overwrite table roam_user_list_maoming partition (month_id = '$v_month_id',day_id='$v_day_id')
  SELECT A.*,
    ...
    left join hy_is_transfer_call H
